...
/**
 * 角色用户管理（路由 {@code admin/role-user}）：左侧角色列表，右侧当前角色下的用户（搜索 + 分页排序），
 * 可从角色移除选中用户、或弹窗添加用户到当前角色。
 */
...
public class RoleUserModel extends AppBoxAdminPageBase {
    ...
    public RoleUserModel(RoleRepository roleRepository, UserRepository userRepository) {
    ...
    public void Page_Load(Object sender, EventArgs e) {
        if (!isPostBack()) {
            btnNew.setEnabled(checkPower("CoreRoleUserNew"));
            boolean powerDelete = checkPower("CoreRoleUserDelete");
    ...
    /** 把这些用户从当前角色移除（用户是角色关系的维护方，改用户的角色集合）。 */
