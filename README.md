...
项目文件已经声明 `com.fineui:fineui-java` 依赖。正常联网时直接运行 `mvn spring-boot:run`，Maven 即会从中央仓库下载。

> **离线备选**：内网上不了公网、或公司私有仓库没有镜像中央仓库时，从 <https://fineui.com/fans/>
> 下载 FineUI.Java 社区版【类库】包，解压后运行 `install.bat`（Windows）或 `sh install.sh`。
...
浏览器访问 **http://localhost:8082/** （注意端口 8082），用 admin / admin 登录。
...
1. **`Could not find artifact com.fineui:fineui-java`**：先检查 Maven 网络、代理与中央仓库镜像；内网环境按第 2 节使用离线【类库】包。
...
5. **H2 控制台报 `Database "C:/Users/xxx/test" not found`**：没替换表单里预填的默认 JDBC URL，见第 3 节。
...
- 客户端点击事件统一使用 `ClickHandler` 引用页面或公共脚本中的具名函数。
- 批量删除和启停操作使用 `hasSelection`、`getSelectedRows`、`F.confirm`、`F.rawHtml` 与 `F.customEvent` 组成清晰的客户端链路。
## 仓库与依赖边界

本仓库是 FineUI.Java.AppBox 的唯一真相源。项目文件已声明从 Maven Central 获取的 `com.fineui:fineui-java` 依赖；仓库不提交 FineUI DLL、JAR 或框架源码。
